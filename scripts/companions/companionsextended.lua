...
function Recruit:init(uuid, json)
  ...
  self.isCustomUniform = json.isCustomUniform or false
...
function Recruit:toJson()
  ...
  json.isCustomUniform = self.isCustomUniform
...
function recruitSpawner:store()
  return {
	  ...
	  customUniforms = self.customUniforms
    }
end
...
function recruitSpawner:load(companions, stored)
  ...
  self.customUniforms = stored.customUniforms or {}
  self:markDirty()
end
...
function recruitSpawner:_updateRecruits(recruits, persistentRecruits, dt)
  for uuid, recruit in pairs(recruits) do
    ...
    if recruit.isCustomUniform then		
		recruit:setUniform(self.customUniforms[uuid])		
	else		
...
function recruitSpawner:dismiss(recruitUuid)
  ...
  self.followers[recruitUuid] = nil
  self.shipCrew[recruitUuid] = nil
  self.customUniforms[recruitUuid] = nil

  self:markDirty()
end

		
function recruitSpawner:setCustomUniform(recruitUuid, uniform)		
  local recruit = self:getRecruit(recruitUuid)		
  if not recruit then		
    sb.logInfo("Cannot set uniform for unknown recruit %s", recruitUuid)		
    return		
  end		
		
  if uniform then		
    self.customUniforms[recruitUuid] = uniform		
	recruit:setUniform(self.customUniforms[recruitUuid])		
	recruit.isCustomUniform = true		
  else		
	self.customUniforms[recruitUuid] = nil		
	recruit.isCustomUniform = false		
	end		
